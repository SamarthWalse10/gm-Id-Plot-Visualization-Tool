...
- **Multiple Channel Lengths:** Compares maximum across 10 channel lengths per technology.
- **Real-time Interpolation:** Cubic-spline interpolation for smooth curves & accurate intersections.
- **Guide Lines:** Optional vertical/horizontal reference lines with auto intersection detection.
...
1. Open the notebook for the technology you want.
   *(Note: You must first export the characterization data from Cadence Virtuoso. These files are not uploaded here due to their large size.)*
...
*(You may need to adjust these based on the lengths exported from your Cadence parametric analysis)*
...
- **Missing data points:** Check CSV formatting and column names (case-sensitive). Make sure numeric columns contain valid values.
